import { cn } from "../lib/utils";
import { Lock } from "lucide-react";
import {
    IconAdjustmentsBolt,
    IconCloud,
    IconEaseInOut,
    IconHeart,
    IconHelp,
    IconRouteAltLeft,
    IconTerminal2,
} from "@tabler/icons-react";



export function FeaturesSectionDemo() {


    const features = [
        {
            title: "Multi-Language Support",
            description: "Write and execute code in multiple programming languages effortlessly.",
            icon: <IconTerminal2 />,
        },
        {
            title: "Real-Time Collaboration",
            description: "Collaborate with your team in real-time using shared code rooms.",
            icon: <IconRouteAltLeft />,
        },
        {
            title: "One-Click Code Sharing",
            description: "Share your code instantly with anyone using a single link.",
            icon: <IconEaseInOut />,
        },
        {
            title: "100% Uptime Guarantee",
            description: "Enjoy uninterrupted coding with a highly reliable platform.",
            icon: <IconCloud />,
        },
        {
            title: "Secure and Private",
            description: "Secure authentication ensures your code is always private.",
            icon: <Lock />,
        },
        {
            title: "Customizable Workspace",
            description: "Personalize your coding environment with ease.",
            icon: <IconAdjustmentsBolt />,
        },
        {
            title: "Seamless Code Import/Export",
            description: "Easily import and export code between projects and tools.",
            icon: <IconHelp />,
        },
        {
            title: "Loved and Trusted by Users",
            description: "We ensures and maintain the privacy of your code.",
            icon: <IconHeart />,
        },
    ];


    return (
        (<div
            className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4  relative z-10 bg-black my-28 max-w-7xl mx-auto">
            {features.map((feature, index) => (
                <Feature key={feature.title} {...feature} index={index} />
            ))}
        </div>)
    );
}

const Feature = ({
    title,
    description,
    icon,
    index
}) => {
    return (
        (<div
            className={cn(
                "flex flex-col lg:border-r  py-10 relative group/feature bg-neutral-900 border-neutral-800",
                (index === 0 || index === 4) && "lg:border-l border-neutral-800",
                index < 4 && "lg:border-b border-neutral-800"
            )}>
            {index < 4 && (
                <div
                    className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-t  from-neutral-800 to-transparent pointer-events-none" />
            )}
            {index >= 4 && (
                <div
                    className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-b  from-neutral-800 to-transparent pointer-events-none" />
            )}
            <div
                className="mb-4 relative z-10 px-10 text-neutral-400">
                {icon}
            </div>
            <div className="text-lg font-bold mb-2 relative z-10 px-10">
                <div
                    className="absolute left-0 inset-y-0 h-6 group-hover/feature:h-8 w-1 rounded-tr-full rounded-br-full bg-neutral-700 group-hover/feature:bg-blue-500 transition-all duration-200 origin-center" />
                <span
                    className="group-hover/feature:translate-x-2 transition duration-200 inline-block text-neutral-100">
                    {title}
                </span>
            </div>
            <p
                className="text-sm text-neutral-300 max-w-xs relative z-10 px-10">
                {description}
            </p>
        </div>)
    );
};
